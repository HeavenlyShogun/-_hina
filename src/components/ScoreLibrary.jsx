...
const ScoreLibrary = memo(({ user, savedScores, onLoadScore, onClearAll, onDeleteScore, onConnectCloud, cloudStatus }) => (
  <div className="bg-black/40 border border-white/5 rounded-[40px] p-6 flex flex-col h-fit max-h-[500px] backdrop-blur-sm shadow-inner relative">
    ...
    {cloudStatus === 'ready' && !user && <div className="absolute inset-0 bg-black/60 rounded-[40px] backdrop-blur-md z-10 flex items-center justify-center text-xs text-white/50 font-bold tracking-widest">?Ｙ?璅∪?</div>}
    <div className="flex items-center justify-between mb-6 px-2">
      <div className="flex items-center gap-2 text-emerald-400 font-black text-[10px] tracking-[0.2em] uppercase"><UploadCloud size={16} /> ?脩垢璅?摨?<span className="text-[8px] opacity-50 ml-1">(暺??喳??)</span></div>
      <button onClick={onClearAll} className="text-rose-400/50 hover:text-rose-400 transition-colors p-1" title="皜征?渲?摨?"><ListX size={14} /></button>
    </div>
      ...
      {savedScores.length === 0 ? <div className="text-center py-20 opacity-10 text-[10px] uppercase tracking-widest">Library Empty</div> : savedScores.map((saved) => (
        <div key={saved.id} onClick={() => onLoadScore(saved)} className="group bg-white/[0.03] p-4 rounded-3xl border border-white/10 hover:bg-emerald-500/20 hover:border-emerald-500/40 transition-all relative flex items-center justify-between cursor-pointer">
          <div className="flex-1 overflow-hidden pr-2">
            <div className="text-sm font-bold text-emerald-50 truncate">{saved.title === saved.id ? saved.title : saved.id}</div>
          ...
          <div className="flex items-center gap-2 transition-opacity">
            <div className="bg-emerald-500/20 text-emerald-400 p-2 rounded-xl group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-md" title="頛甇日蝡舀?霅?"><FolderOpen size={16} /></div>
            <button onClick={(event) => { event.stopPropagation(); onDeleteScore(saved.id); }} className="p-2 text-rose-400/40 hover:text-rose-400 hover:bg-rose-500/20 rounded-xl transition-all" title="?芷"><Trash2 size={16} /></button>
          </div>
        </div>
      ))}
    </div>
