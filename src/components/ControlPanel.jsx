...
const ControlPanel = memo(({ bpm, setBpm, timeSigNum, setTimeSigNum, timeSigDen, setTimeSigDen, charResolution, setCharResolution, vol, setVol, tone, setTone, reverb, onToggleReverb, globalKeyOffset, setGlobalKeyOffset }) => (
  <section className="z-30 w-full max-w-6xl my-10 flex flex-col gap-4 px-6">
    <div className="flex flex-wrap justify-center items-center gap-4">
      <div className="flex items-center gap-3 bg-white/[0.03] border border-white/5 rounded-full px-5 py-2.5 backdrop-blur-md shadow-inner" title="隤踵?湧?瞍??漲">
      ...
      <div className="flex items-center gap-2 bg-emerald-950/30 border border-emerald-500/20 rounded-full px-5 py-2.5 backdrop-blur-md">
        ...
        <span className="text-[10px] font-black text-emerald-200/70 uppercase tracking-widest mr-1 hidden sm:inline">??</span>
        ...
        <span className="text-[10px] font-black text-emerald-200/70 uppercase tracking-widest mr-1 md:mr-2">1摮?=</span>
        <select value={charResolution} onChange={(event) => setCharResolution(Number(event.target.value))} className="bg-transparent outline-none text-[11px] text-emerald-400 font-black cursor-pointer">
          <option value={4} className="bg-slate-900">4?蝚?</option>
          <option value={8} className="bg-slate-900">8?蝚?</option>
          <option value={16} className="bg-slate-900">16?蝚?</option>
          <option value={32} className="bg-slate-900">32?蝚?</option>
        </select>
      </div>
    ...
    <div className="flex flex-wrap justify-center items-center gap-4">
      <div className="flex items-center gap-3 bg-white/[0.03] border border-white/5 rounded-full px-5 py-2.5 backdrop-blur-md">
        ...
        <select value={tone} onChange={(event) => setTone(event.target.value)} className="bg-transparent outline-none text-[11px] font-black text-emerald-100 uppercase cursor-pointer">
          <option value="piano" className="bg-[#0f172a]">?潛 (Piano)</option>
          <option value="flute" className="bg-[#0f172a]">蝚? (Flute)</option>
          <option value="tongue-drum" className="bg-[#0f172a]">蝛粹?曌?(Steel Tongue)</option>
          <option value="lyre-long" className="bg-[#0f172a]">憸函銋帘??(?瑟???</option>
          <option value="lyre-short" className="bg-[#0f172a]">憸函銋帘??(?凋???</option>
          <option value="classic" className="bg-[#0f172a]">蝬?餃???(銝?瘜?</option>
        </select>
      </div>
