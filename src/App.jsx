...
function AppContent({
  ...
  const workspaceSections = useMemo(() => ([
    { id: 'playback-preview', label: '????', caption: 'Live Preview' },
    { id: 'editor', label: '譜面編輯', caption: 'Score Editor' },
    { id: 'cloud-library', label: '????', caption: 'Firebase / Cloud' },
    { id: 'converter', label: '????', caption: 'Converter' },
  ]), []);
  ...
  const handleConnectCloud = useCallback(async () => {
    ...
    if (!result) {
      showToast(cloudError || 'Firebase ?????', 'error');
      return;
    }
    showToast('?????', 'success');
  ...
  const handleLoadScore = useCallback(async (savedScore) => {
    ...
    stopAll();
    showToast(`??? ${fullScore.title}`, 'success');
  ...
  const handleImportLocal = useCallback(async (event) => {
    ...
    try {
      if (files.length === 1) {
        ...
        stopAll();
        showToast(`??? ${source.title}`, 'success');
      } else {
  ...
  const handlePlayFeaturedScore = useCallback(async (featuredScore) => {
    ...
    stopAll();
    showToast(`???? ${score.displayTitle ?? score.title}`, 'success');
